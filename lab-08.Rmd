...
```{r}
missing_values_RainTomorrow <- sum(is.na(weatherAUS$RainTomorrow))
cat("Number of missing values in RainTomorrow:", missing_values_RainTomorrow, "\n")
weather_noNA <- weatherAUS %>% filter(!is.na(RainTomorrow))
...
```{r}
cities_rain_days <- weather_noNA %>%
  group_by(Location) %>%
  summarise(total_rain_days = sum(RainToday == "Yes", na.rm = TRUE)) %>%
  arrange(desc(total_rain_days))
head(cities_rain_days, n = 5)
